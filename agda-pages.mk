...
# Files with other extensions are from unsupported kinds of literate Agda,
# and are ignored.

# Front matter with a `title` value is prepended to all pages. For pages
# generated from literate Markdown Agda source files, any existing front matter
# is preserved (it should not already provide a value for `title`), and a
# top-level heading is added, if missing.
...
# Front matter: cat agda/Demo/Literate/Metadata.lagda.md | sd '\A---\n((.|\n)*?)^---\n(.|\n)*\z' '$1'
...
# `make gen-lagda` generates LATEX/*.lagda files from *.lagda.md files using pandoc
